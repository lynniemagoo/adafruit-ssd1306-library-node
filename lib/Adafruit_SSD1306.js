...
class Adafruit_SSD1306 extends Display_Base {
    ...
    */
    ...
    constructor(options) {
        ...
        self._contrast = 0x8F;
        self._colOffset = extractOption(options, "colOffset", 0);
    ...
    */
    ...
    begin() {
        ...
        self._ssd1306_command1(SSD1306_SET_DISPLAY_OFFSET, [displayOffset & 0xFF]);
        self._ssd1306_command1(SSD1306_SET_START_LINE, [startLine & 0xFF]);
    ...
    display() {
        ...
        for (let page = pageOffset, index = 0x00; page < pageEnd; page++,index+=bytesPerPage) {
            self._ssd1306_command1(SSD1306_COLUMN_ADDR, [
                    col & 0xFF,                     // column start address
                    (col + bytesPerPage - 1) & 0xFF // column end address
                ]);
            self._ssd1306_command1(SSD1306_PAGE_ADDR, [
                    page & 0xFF,                    // page start address
                    (page + pagesPerWrite) & 0xFF   // page end address
                ]);
            // write buffer data one page at a time.
            self._ssd1306_data(buffer.subarray(index, index + (bytesPerPage * pagesPerWrite)));
        }
        return self;
    }
