...
class Adafruit_SSD1306 extends Display_Base {
    ...
    */
    ...
    async _preStartup() {
        const self = this;
        await self._hardwareStartup();   // (setup SPI, I2C)
        await self._hardwareReset();     // (hardware reset for SPI)
        return self;
    }
    ...
    async _preShutdown() {
        const self = this;
        await self._ssd1306_command1(SSD1306_DEACTIVATE_SCROLL); // DeactivateScroll
        await self.setDisplayOnOff(false);  // Turn off screen
        return self;
    }
    ...
    async _postShutdown() {
        const self = this;
        await self._hardwareShutdown(); // (release SPI, I2C hardware)
        return self;
    }
    ...
    */
    /**************************************************************************/
    begin() {
        const self = this, rotation = self.rotation, vccSelection = self._vccSelection;
        ...
        // send display memory prior to show.
        self.clearDisplay();
        self.display();
